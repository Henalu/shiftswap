-- Let a shift publication use a custom job position name when the
-- company catalogue does not have that position yet.

ALTER TABLE public.shifts
  ADD COLUMN IF NOT EXISTS custom_job_position_name TEXT NULL;

ALTER TABLE public.shifts
  DROP CONSTRAINT IF EXISTS shifts_job_position_source_check;

ALTER TABLE public.shifts
  ADD CONSTRAINT shifts_job_position_source_check
  CHECK (job_position_id IS NULL OR custom_job_position_name IS NULL);

ALTER TABLE public.shifts
  DROP CONSTRAINT IF EXISTS shifts_custom_job_position_name_check;

ALTER TABLE public.shifts
  ADD CONSTRAINT shifts_custom_job_position_name_check
  CHECK (
    custom_job_position_name IS NULL
    OR (
      char_length(custom_job_position_name) BETWEEN 2 AND 80
      AND custom_job_position_name = btrim(custom_job_position_name)
      AND custom_job_position_name !~ '[[:space:]]{2,}'
      AND custom_job_position_name ~ '^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+( [A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)*$'
      AND lower(custom_job_position_name) !~ '(^| )(cabron|cabrón|cojones|cono|coño|culo|gilipollas|hijo puta|hijoputa|joder|maricon|maricón|mierda|polla|puta|putas|puto|putos|zorra)( |$)'
    )
  );
...
DECLARE
  ...
  normalized_custom_position TEXT;
BEGIN
  normalized_custom_position := NULLIF(
    btrim(
      regexp_replace(
        COALESCE(NEW.custom_job_position_name, ''),
        '[[:space:]]+',
        ' ',
        'g'
      )
  ...
  NEW.custom_job_position_name := normalized_custom_position;
  ...
  IF NEW.job_position_id IS NOT NULL AND NEW.custom_job_position_name IS NOT NULL THEN
    RAISE EXCEPTION 'Shift publication cannot use a catalogue job position and a custom job position at the same time';
  END IF;

  IF NEW.custom_job_position_name IS NOT NULL THEN
    IF char_length(NEW.custom_job_position_name) NOT BETWEEN 2 AND 80
       OR NEW.custom_job_position_name !~ '^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+( [A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)*$'
       OR lower(NEW.custom_job_position_name) ~ '(^| )(cabron|cabrón|cojones|cono|coño|culo|gilipollas|hijo puta|hijoputa|joder|maricon|maricón|mierda|polla|puta|putas|puto|putos|zorra)( |$)' THEN
      RAISE EXCEPTION 'Shift publication custom job position is not valid';
    END IF;
  END IF;
...
BEFORE INSERT OR UPDATE OF user_id, department_id, job_position_id, custom_job_position_name
ON public.shifts
FOR EACH ROW
...
CREATE POLICY "Users can create own shifts"
  ...
  WITH CHECK (
    ...
    AND (
      ...
      OR (
        job_position_id IS NULL
        AND char_length(custom_job_position_name) BETWEEN 2 AND 80
        AND custom_job_position_name = btrim(custom_job_position_name)
        AND custom_job_position_name !~ '[[:space:]]{2,}'
        AND custom_job_position_name ~ '^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+( [A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)*$'
        AND lower(custom_job_position_name) !~ '(^| )(cabron|cabrón|cojones|cono|coño|culo|gilipollas|hijo puta|hijoputa|joder|maricon|maricón|mierda|polla|puta|putas|puto|putos|zorra)( |$)'
